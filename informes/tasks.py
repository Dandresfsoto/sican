...
def matriz_chequeo_vigencia_2017(email,id_contrato):
    ...
    proceso = "REV-INF06"
    informe = InformesExcel.objects.create(usuario = usuario,nombre=nombre,progreso="0%")
    informe.nombre = "Matriz de chequeo - " + contrato.nombre
    informe.save()
    fecha = informe.creacion
    output = StringIO()
    dict_productos = []
    ...
    for beneficiario in BeneficiarioVigencia2017.objects.filter(grupo__in = grupos):
        ...
        ws.cell(row=contadores[str(id_diplomado)], column=2, value = beneficiario.dane_sede.dane_sede)
        ws.cell(row=contadores[str(id_diplomado)], column=3, value = beneficiario.dane_sede.nombre_sede.upper())
        ws.cell(row=contadores[str(id_diplomado)], column=4, value = beneficiario.dane_sede.dane_ie)
        ws.cell(row=contadores[str(id_diplomado)], column=5, value = beneficiario.dane_sede.nombre_ie.upper())
        ws.cell(row=contadores[str(id_diplomado)], column=6, value = beneficiario.dane_sede.municipio.codigo_municipio)
        ws.cell(row=contadores[str(id_diplomado)], column=7, value = beneficiario.dane_sede.municipio.nombre.upper())
        ws.cell(row=contadores[str(id_diplomado)], column=8, value = beneficiario.dane_sede.municipio.departamento.codigo_departamento)
        ws.cell(row=contadores[str(id_diplomado)], column=9, value = beneficiario.dane_sede.municipio.departamento.nombre.upper())
        ws.cell(row=contadores[str(id_diplomado)], column=10, value = beneficiario.dane_sede.secretaria.nombre.upper())
        ws.cell(row=contadores[str(id_diplomado)], column=11, value = beneficiario.dane_sede.zona.upper())
        ...
        for entregable in Entregable.objects.filter(sesion__nivel__diplomado__id = id_diplomado):
            ...
            if EvidenciaVigencia2017.objects.filter(entregable=entregable).filter(beneficiarios_cargados = beneficiario).count() > 0:
                ws.cell(row=contadores[str(id_diplomado)], column= 25 + int(entregable.numero), value = "SI")
                ws.cell(row=contadores[str(id_diplomado)], column=25 + int(entregable.numero)).style = enviado

            else:
                ws.cell(row=contadores[str(id_diplomado)], column=25 + int(entregable.numero), value="NO")

        contadores[str(id_diplomado)] += 1


    if contadores['1'] == 6:
        ws_innovatic.sheet_state = 'hidden'
    if contadores['2'] == 6:
        ws_tecnotic.sheet_state = 'hidden'
    if contadores['3'] == 6:
        ws_directic.sheet_state = 'hidden'
    if contadores['4'] == 6:
        ws_escuelatic.sheet_state = 'hidden'
